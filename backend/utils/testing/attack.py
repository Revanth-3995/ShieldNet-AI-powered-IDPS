...
def main():
    parser = argparse.ArgumentParser(description="ShieldNet Live Attacker")
    ...
    parser.add_argument("--threads", type=int, default=10, help="Number of concurrent attack streams")
    args = parser.parse_args()

    print(f"[*] Initiating live attack on {args.target}:{args.port}")
    print(f"[*] Launching {args.threads} attack threads...")

    threads = []
    for i in range(args.threads):
        t = threading.Thread(target=attack_thread, args=(args.target, args.port, 1.0 + (i * 0.5)))
        threads.append(t)
        t.start()
        time.sleep(0.1)
